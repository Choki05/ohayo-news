...
**GitHub TrendingとHacker News**は、公式ページ自体をWeb検索経由で直接取得できたため、比較的近い結果が得られました。ただし、正確な今日のページにたどり着くまでに10回近い検索を要しており、このツール（1回のAPI呼び出しで即座に取得）に比べると大幅に時間と手間がかかっています。

**ZennとQiitaは、公式のリアルタイム・トレンドAPIが存在しないため、うまくいきませんでした。**

- Zennでは、非公式API（`zenn-api.vercel.app`）に頼らざるを得なかった。いつサービスが終了してもおかしくない、個人が作ったツールです。
